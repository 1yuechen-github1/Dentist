...
namespace DentistDatabase
{
    public partial class 初诊 : Form
    {
        ...
        {
            String sqlText = @"SELECT  [牙位-患者表].种植牙位, 初诊.[就诊记录(时间+日期)], 初诊.初诊日期,  初诊.是否取初诊印模, 初诊.是否拍初诊CBCT, 初诊.初诊CBCT资源链接, 初诊.是否拍初诊小牙片, 初诊.初诊小牙片资源链接, 
 初诊.是否拍初诊全景片, 初诊.初诊全景片资源链接, 初诊.缺牙位点, 初诊.是否口扫, 初诊.口扫资源链接, 初诊.是否面扫, 初诊.面扫资源链接, 初诊.是否电子面弓, 初诊.电子面弓资源链接, 初诊.病历, 初诊.病历图片资源链接  " +
        ...
        public static void Save(String ID)
        {
            ...
            changeDt = dt.GetChanges(); //private static System.Data.DataTable dt = null;
                                        //null表示未被实例化（2.16周四）
            if (changeDt == null)
                return;
            foreach (DataRow dr in changeDt.Rows)
            {
                ...
                if (dr.RowState == System.Data.DataRowState.Added) //增加
                {
                    strSQL = @"IF((SELECT COUNT(*)
                                    FROM [牙位-患者表]
                                    WHERE 牙位ID = '" + ID + dr["种植牙位"].ToString() + @"') = 1 AND (SELECT COUNT(*) 
                                    FROM 初诊
                                    WHERE 牙位ID = '" + ID + dr["种植牙位"].ToString() + @"') = 1)
                                BEGIN
                                    RAISERROR('已存在数据', 16, 1)
                                END
                                ELSE
                               BEGIN
                                ...
                                INSERT INTO [dbo].[初诊]
                                     ...
                                     VALUES
                                           ...
                                           ,'" + dr["初诊CBCT资源链接"].ToString() + @"'
                                           ,'" + dr["是否拍初诊小牙片"].ToString() + @"'
                                           ,'" + dr["初诊小牙片资源链接"].ToString() + @"'
                                           ,'" + dr["是否拍初诊全景片"].ToString() + @"'
                                           ,'" + dr["初诊全景片资源链接"].ToString() + @"'
                                           ,'" + dr["缺牙位点"].ToString() + @"'
                                           ,'" + dr["是否口扫"].ToString() + @"'
                                           ,'" + dr["口扫资源链接"].ToString() + @"' 
                                           ,'" + dr["是否面扫"].ToString() + @"'
                                           ,'" + dr["面扫资源链接"].ToString() + @"'
                                           ,'" + dr["是否电子面弓"].ToString() + @"'
                                           ,'" + dr["电子面弓资源链接"].ToString() + @"'        
                                           
                                           ,'" + dr["病历"].ToString() + @"'
                                            ,'" + dr["病历图片资源链接"].ToString() + @"
                                           )
                                      END";
                }
                ...
                else if (dr.RowState == System.Data.DataRowState.Modified) //修改
                {
                    strSQL = @"UPDATE [初诊]
                            SET [就诊记录(时间+日期)] = '" + dr["就诊记录(时间+日期)"].ToString() + @"'
                              ...
                              ,[初诊CBCT资源链接] = '" + dr["初诊CBCT资源链接"].ToString() + @"'
                              ,[是否拍初诊小牙片] = '" + dr["是否拍初诊小牙片"].ToString() + @"'
                              ,[初诊小牙片资源链接] = '" + dr["初诊小牙片资源链接"].ToString() + @"'
                              ,[是否拍初诊全景片] = '" + dr["是否拍初诊全景片"].ToString() + @"'
                              ,[初诊全景片资源链接] = '" + dr["初诊全景片资源链接"].ToString() + @"'
                              ,[缺牙位点] = '" + dr["缺牙位点"].ToString() + @"'
                              ,[是否口扫] = '" + dr["是否口扫"].ToString() + @"'
                              ,[口扫资源链接] = '" + dr["口扫资源链接"].ToString() + @"' 
                              ,[是否面扫] = '" + dr["是否面扫"].ToString() + @"'
                              ,[面扫资源链接] = '" + dr["面扫资源链接"].ToString() + @"'
                              ,[是否电子面弓] = '" + dr["是否电子面弓"].ToString() + @"'
                              ,[电子面弓资源链接] = '" + dr["电子面弓资源链接"].ToString() + @"'
                              ,[病历] = '" + dr["病历"].ToString() + @"'
                              ,[病历图片资源链接] = '" + dr["病历图片资源链接"].ToString() + @"'
